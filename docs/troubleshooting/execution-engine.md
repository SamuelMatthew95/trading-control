...
## Regime-aware execution gate — marginal longs are filtered at the execution boundary in a bearish regime (issues #330 / #331 / #332)

**Symptom:** The learning loop filed three converging proposals — a `regime_adjustment` to *"implement a regime detection system and adjust the model's parameters"* in a `bearish` regime (issue #330), a `regime_adjustment` that the model is *"over-trading, resulting in a high number of losing trades… reducing the EXECUTION_DECISION_THRESHOLD"* (issue #331), and a `code_change` that *"the current win rate is low"* (issue #332). All three describe the same loss pattern: in a bearish tape the book keeps taking marginal momentum longs, which over-trades and drags the win rate down.

**Root cause:** The regime-aware posture from #326/#328 tightens sizing, stops, take-profit, the daily-loss limit, and the *reasoning-node* entry confidence floor (`decide_policy`). But the **execution gate itself** — `EXECUTION_DECISION_THRESHOLD`, the weighted `signal·0.5 + reasoning·0.3 + historical·0.2` score every decision on `STREAM_DECISIONS` must clear — was regime-blind. A marginal MOMENTUM long (`signal ≈ 0.55 → final_score ≈ 0.56`) cleared the `0.55` gate in every regime, including a falling market, and the LLM decision path (which does not run `decide_policy`) had no entry floor at all on this last boundary.

**Fix:** A new regime-conditional parameter in the single-source-of-truth policy module — `regime_risk.execution_threshold(regime, default, *, is_long)` — raises the execution gate a **new long** must clear to `RISK_OFF_EXECUTION_DECISION_THRESHOLD = 0.65` (vs the `0.55` default) in a `RISK_OFF` regime. `ExecutionEngine._evaluate_pre_execution_gates` reads the **cache-only** macro regime (`market_intel.read_cached_macro_regime` — no Alpaca fetch) for BUY actions only and applies it before the score gate: a marginal long (`final_score ≈ 0.56`) is now gated in a bearish tape, while STRONG_MOMENTUM conviction (`final_score ≈ 0.76`) still trades. This is the execution-boundary complement to #328's reasoning-node floor — it catches LLM-path decisions the deterministic policy floor never sees. **Fail-safe invariant (same as #326/#328):** resolved as `max(default, RISK_OFF_EXECUTION_DECISION_THRESHOLD)` so the gate can only ever *raise* the bar — never lower an operator/control-plane override; SELL exits keep the default gate (the regime cache is not consulted for non-BUY actions) so the book can always de-risk; every non-risk-off / unknown / missing / malformed regime falls back to the default.

**Regression test:** `tests/core/test_regime_risk.py::test_execution_threshold_raises_only_for_long_in_risk_off` / `::test_execution_threshold_never_lowers_a_stricter_default` / `::test_risk_off_constants_are_strictly_more_conservative`, `tests/agents/test_execution_engine.py::test_execution_gate_blocks_marginal_long_in_risk_off_regime` / `::test_execution_gate_passes_marginal_long_in_neutral_regime` / `::test_execution_gate_does_not_tighten_sell_exit_in_risk_off`
