...
class TestDateTimeTimezone:
    ...
    async def test_order_datetime_columns_are_timezone_aware(self):
        ...
        assert (
            created_at_col.type.timezone
        ), "Order.created_at should have timezone=True"
        ...
        assert hasattr(
            default_created, "arg"
        ), "Order.created_at default should be callable"
        test_datetime = default_created.arg(None)  # Pass context=None for testing
        assert (
            test_datetime.tzinfo is not None
        ), "Order.created_at default should be timezone-aware"
        assert (
            test_datetime.tzinfo == timezone.utc
        ), "Order.created_at should use UTC timezone"
    ...
    async def test_system_metric_datetime_columns_are_timezone_aware(self):
        ...
        # Assert timezone=True
        assert (
            timestamp_col.type.timezone
        ), "SystemMetric.timestamp should have timezone=True"

        # Test default function produces timezone-aware datetime
        default_timestamp = SystemMetric.timestamp.default
        assert hasattr(
            default_timestamp, "arg"
        ), "SystemMetric.timestamp default should be callable"
        test_datetime = default_timestamp.arg(None)  # Pass context=None for testing
        assert (
            test_datetime.tzinfo is not None
        ), "SystemMetric.timestamp default should be timezone-aware"
        assert (
            test_datetime.tzinfo == timezone.utc
        ), "SystemMetric.timestamp should use UTC timezone"
    ...
    async def test_insert_system_metric_with_aware_datetime_no_error(self):
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                result = await session.execute(
                    text(
                        "SELECT COUNT(*) FROM system_metrics WHERE metric_name = :name"
                    ),
                    {"name": "test_metric"},
                )
                count = result.scalar()
                assert (
                    count == 1
                ), "SystemMetric should be saved with timezone-aware datetime"
        finally:
            await engine.dispose()
    ...
    def test_no_datetime_utcnow_usage_in_models(self):
        ...
        ), "datetime.utcnow should not be used in models"
        assert (
            "datetime.now(timezone.utc)" in source
        ), "Should use timezone-aware datetime.now"
