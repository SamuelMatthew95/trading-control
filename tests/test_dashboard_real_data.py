...
async def test_pnl_aggregation(api_client):
    ...
    async with AsyncSessionLocal() as session:
        for v in [120.0, 340.0, 80.0, 210.0, 95.0]:
            session.add(Run(task_id=f"won-{v}", task_type="pharma earnings", status="won", pnl=v, step_count=4, decision_json="{}", trace_json="[]", created_at=now))
        for v in [-50.0, -30.0]:
            session.add(Run(task_id=f"fail-{v}", task_type="pharma earnings", status="failed", pnl=v, step_count=4, decision_json="{}", trace_json="[]", created_at=now))
        await session.flush()
        for c in [4.5, 5.0, 5.0]:
            session.add(TraceStep(run_id=1, node_name="n", token_cost_usd=c, created_at=now))
        await session.commit()
...
async def test_tool_thrashing_detection(api_client):
    now = TEST_REFERENCE_DT
    async with AsyncSessionLocal() as session:
        run = Run(task_id="thrash", task_type="x", status="won", pnl=1, step_count=5, decision_json="{}", trace_json="[]", created_at=now)
        session.add(run)
        await session.flush()
...
async def test_memory_guard_effectiveness(api_client):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for i, pnl in enumerate(pnls):
            run = Run(task_id=f"g{i}", task_type="x", status="won" if pnl > 0 else "failed", pnl=pnl, step_count=1, decision_json="{}", trace_json="[]", created_at=now)
            session.add(run)
            await session.flush()
...
async def test_signal_auto_generation(api_client):
    now = TEST_REFERENCE_DT
    async with AsyncSessionLocal() as session:
        session.add(Run(task_id='loss', task_type='x', status='failed', pnl=-600, step_count=1, decision_json='{}', trace_json='[]', created_at=now-timedelta(minutes=30)))
        await session.commit()
...
async def test_signal_dismiss(api_client):
    async with AsyncSessionLocal() as session:
        session.add(Signal(id='sig-x', priority='info', message='m', action_label='Dismiss', action_type='dismiss', run_id=None, dismissed=False))
        await session.commit()
...
async def test_insight_confidence_fields(api_client):
    async with AsyncSessionLocal() as session:
        session.add(Insight(run_id=1, tag='Learned', confidence=0.8, summary='ok', supporting_run_count=2))
...
async def test_dashboard_pnl_today_isolation(api_client):
    now = TEST_REFERENCE_DT
    async with AsyncSessionLocal() as session:
        session.add(Run(task_id='old', task_type='x', status='won', pnl=500, step_count=1, decision_json='{}', trace_json='[]', created_at=now-timedelta(days=2)))
...
async def test_empty_database_returns_zeros(api_client):
    now = TEST_REFERENCE_DT
    assert (await api_client.get('/dashboard/pnl', params={'reference_dt': now.isoformat()})).status_code == 200
    assert (await api_client.get('/dashboard/learning-velocity', params={'reference_dt': now.isoformat()})).status_code == 200
    assert (await api_client.get('/dashboard/health-signals', params={'reference_dt': now.isoformat()})).status_code == 200
    assert (await api_client.get('/dashboard/run-summary', params={'reference_dt': now.isoformat()})).status_code == 200
...
async def test_passk_trend_with_insufficient_data(api_client):
    ...
    async with AsyncSessionLocal() as session:
        for d in [1, 0]:
            session.add(Run(task_id=f'i{d}', task_type='x', status='won', pnl=1, step_count=1, decision_json='{}', trace_json='[]', created_at=now-timedelta(days=d)))
        await session.commit()
    res = await api_client.get('/dashboard/learning-velocity', params={'reference_dt': now.isoformat()})
    body = res.json()
...
async def test_signal_deduplication(api_client):
    now = TEST_REFERENCE_DT
    async with AsyncSessionLocal() as session:
        session.add(Run(task_id='loss2', task_type='x', status='failed', pnl=-600, step_count=1, decision_json='{}', trace_json='[]', created_at=now-timedelta(minutes=30)))
        await session.commit()
    await generate_signals(reference_dt=now)
    await generate_signals(reference_dt=now)
...
async def test_baseline_slippage_not_overwritten(api_client):
    now = TEST_REFERENCE_DT
    async with AsyncSessionLocal() as session:
        session.add(TaskTypeBaseline(task_type='pharma_earnings', baseline_slippage=0.15))
        session.add(Run(task_id='b1', task_type='pharma_earnings', status='won', pnl=1, step_count=1, actual_slippage=0.15, decision_json='{}', trace_json='[]', created_at=now-timedelta(days=1)))
        session.add(Run(task_id='b2', task_type='pharma_earnings', status='won', pnl=1, step_count=1, actual_slippage=0.08, decision_json='{}', trace_json='[]', created_at=now))
        await session.commit()
    async with AsyncSessionLocal() as session:
        baseline = (await session.execute(select(TaskTypeBaseline).where(TaskTypeBaseline.task_type == 'pharma_earnings'))).scalar_one()
...
async def test_run_summary_task_type_filter(api_client):
    ...
    async with AsyncSessionLocal() as session:
        for i in range(3):
            session.add(Run(task_id=f'p{i}', task_type='pharma_earnings', status='won', pnl=1, step_count=1, decision_json='{}', trace_json='[]', created_at=now))
        for i in range(2):
...
async def test_baseline_upsert_is_idempotent(seeded_db):
    ...
    async with AsyncSessionLocal() as session:
        await svc.persist_run(session, {"task_id": "pharma_earnings:1", "decision": {"DECISION": "LONG"}, "trace": [], "actual_slippage": 0.15})
        await svc.persist_run(session, {"task_id": "pharma_earnings:2", "decision": {"DECISION": "LONG"}, "trace": [], "actual_slippage": 0.08})
        await svc.persist_run(session, {"task_id": "pharma_earnings:3", "decision": {"DECISION": "LONG"}, "trace": [], "actual_slippage": 0.01})
        rows = (await session.execute(select(TaskTypeBaseline).where(TaskTypeBaseline.task_type == 'pharma_earnings'))).scalars().all()
        assert len(rows) == 1
...
async def test_correction_verified_after_successful_run(seeded_db):
    ...
    svc = AgentMemoryService()
    async with AsyncSessionLocal() as session:
        session.add(VectorMemoryRecord(store_type='negative-memory', run_id=1, node_name='pharma_earnings', content='bad tool', embedding_json='[]'))
        run = Run(task_id='pharma_earnings:ok', task_type='pharma_earnings', status='won', pnl=100, step_count=1, scoring_status='scored', decision_json='{}', trace_json='[]')
        session.add(run)
        await session.flush()
        await svc.verify_corrections(session, run)
        row = (await session.execute(select(VectorMemoryRecord).where(VectorMemoryRecord.node_name == 'pharma_earnings'))).scalar_one()
