...
class TestNaiveDatetime:
    ...
    async def test_record_system_metric_uses_naive_datetime(self):
        ...
        with patch("api.main.AsyncSessionFactory") as mock_session_factory:
            ...
            if len(call_args) >= 2:
                ...
                sql_params = (
                    call_args[0][1] if isinstance(call_args[0], tuple) else call_args[1]
                )

                # Check that timestamp parameter exists and is naive
                assert (
                    "timestamp" in sql_params
                ), "timestamp parameter should be in SQL params"
                ...
                ), f"timestamp should be naive (tzinfo=None), but got {timestamp.tzinfo}"
    ...
    def test_no_aware_datetimes_in_raw_sql_main_py(self):
        ...
        for i, line in enumerate(lines):
            ...
            if (
                "datetime.now(timezone.utc)" in line
                and ".replace(tzinfo=None)" not in line
            ):
        ...
        ), f"Found timezone-aware datetime.now(timezone.utc) in raw SQL context without .replace(tzinfo=None):\n{chr(10).join(problematic_lines)}"
    ...
    def test_all_naive_datetime_fixes_present(self):
        ...
        for file_path in files_to_check:
            ...
            for i, line in enumerate(lines):
                ...
                if (
                    ".replace(tzinfo=None)" in line
                    and "datetime.now(timezone.utc)" in line
                ):
            ...
            if file_path in [
                ...
                assert (
                    len(fixes_found) > 0
                ), f"No .replace(tzinfo=None) fixes found in raw SQL contexts in {file_path}. Expected at least one fix for raw SQL datetime parameters."
    ...
    async def test_execution_engine_uses_naive_datetime(self):
        ...
        with patch(
            ...
            engine, "_upsert_position"
        ) as mock_upsert, patch.object(
            engine, "_insert_audit_log"
        ) as mock_audit:
            ...
            for call in mock_execute.call_args_list:
                ...
                if "filled_at" in call_args:
                    ...
                    assert isinstance(
                        filled_at, datetime
                    ), "filled_at should be a datetime object"
                    assert (
                        filled_at.tzinfo is None
                    ), f"filled_at should be naive (tzinfo=None), but got {filled_at.tzinfo}"
