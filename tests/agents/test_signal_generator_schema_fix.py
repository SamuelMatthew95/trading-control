...
class TestSignalGeneratorSchemaFix:
    ...
    def test_signal_generator_uses_strategy_id_not_agent_id(self):
        ...
        assert len(agent_id_param_lines) == 0, f"Found agent_id parameter usage: {agent_id_param_lines}"
        
        # Should find strategy_id usage
        strategy_id_lines = [line.strip() for line in lines if '"strategy_id":' in line]
        assert len(strategy_id_lines) >= 2, f"Should find at least 2 strategy_id parameter usages. Found: {strategy_id_lines}"
    ...
    def test_branch_documentation(self):
        ...
        expected_keys = ["branch", "purpose", "issue", "solution", "files_changed", "test_coverage", "verification"]
    ...
    def test_insert_statements_use_correct_columns(self):
        ...
        assert "(id, strategy_id, trace_id" in source_code, "agent_runs INSERT should use strategy_id column"
    ...
    def test_error_message_pattern_fixed(self):
        ...
        agent_runs_context = source_code.split("INSERT INTO agent_runs")[1].split("INSERT INTO")[0] if "INSERT INTO agent_runs" in source_code else ""
        
        assert "agent_id" not in agent_runs_context, f"agent_runs context should not contain agent_id. Context: {agent_runs_context[:200]}..."
        assert "strategy_id" in agent_runs_context, f"agent_runs context should contain strategy_id. Context: {agent_runs_context[:200]}..."
