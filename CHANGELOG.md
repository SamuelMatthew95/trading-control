...
- **Repo-wide adoption of `now_iso()` + four more shared helpers** — a deep audit found the same idioms duplicated far more widely than the first pass caught. `api/utils.py` gained `parse_iso_datetime` / `parse_iso_timestamp` (the `datetime.fromisoformat(s.replace("Z","+00:00"))` idiom, with naive→UTC normalization and uniform error handling — was hand-rolled in ~10 places with subtly different tz/exception handling), `bytes_to_text` (the `x.decode() if isinstance(x, bytes) else …` Redis-decode idiom, ~9 copies), `safe_json_loads` (best-effort JSON decode), and `clamp` (two identical private copies). Every `datetime.now(timezone.utc).isoformat()` call site across `api/` (~180 occurrences in ~50 files) now calls `now_iso()`; the bespoke `_to_text` / `_safe_loads` / `_parse_iso_ts` / `_parse_utc_datetime` / `_parse_since` / `safe_parse_dt` / `_clamp` helpers delegate to the shared ones. Pure mechanical/behaviour-preserving consolidation — full suite green (`tests/core/test_payload_utils.py` covers the new helpers)
