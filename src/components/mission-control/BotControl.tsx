  ...
  return (
    <Card className="bg-card border-border">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
        <CardTitle className="text-base font-semibold text-foreground">Bot Control</CardTitle>
        <Badge 
      ...
      <CardContent className="space-y-4">
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-md px-4 py-2 dark:bg-red-950/30 dark:border-red-800">
            <div className="flex items-center">
              ...
              <span className="text-red-600 dark:text-red-400 text-sm">Could not fetch bot status</span>
              <button 
                onClick={onRetry}
        ...
        <div className="flex items-center justify-between p-4 rounded-lg bg-secondary/50">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${isRunning ? 'bg-success' : 'bg-warning'} animate-pulse`} />
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex items-center justify-between p-3 rounded-lg bg-secondary/30">
            ...
            <Badge variant={health?.data?.database_connected ? 'default' : 'destructive'} className="px-2 py-1 text-xs">
              {health?.data?.database_connected ? 'Connected' : 'Disconnected'}
            </Badge>
          </div>
          <div className="flex items-center justify-between p-3 rounded-lg bg-secondary/30">
            <span className="text-xs text-muted-foreground">Error Rate</span>
            <Badge variant={health?.data?.telemetry?.error_rate === 0 ? 'default' : 'destructive'} className="px-2 py-1 text-xs">
        ...
        {isConfirming && (
          <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/20">
            <p className="text-sm text-destructive leading-relaxed">
              ⚠️ Stopping the bot will immediately halt all trading operations. Make sure this is what you want to do.
            </p>
          </div>
        )}
      </CardContent>
