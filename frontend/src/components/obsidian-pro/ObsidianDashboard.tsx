...
const AgentStatusPulse = memo(
  ({ agentLogs: _agentLogs }: { agentLogs: unknown[] }) => {
    const agents = [
      {
        name: "Reasoning Agent",
        status: "running",
        lastAction: "Analyzed BTC trend",
      },
      {
        name: "Execution Engine",
        status: "running",
        lastAction: "Placed SPY order",
      },
      {
        name: "Learning Service",
        status: "idle",
        lastAction: "Processing trade data",
      },
      {
        name: "IC Updater",
        status: "idle",
        lastAction: "Updated confidence model",
      },
    ];
...
const SystemHealthMetrics = memo(
  ({
    systemMetrics,
  }: {
    systemMetrics: Array<{ metric_name?: string; value?: number }>;
  }) => {
    const costToday =
      systemMetrics.find((m) => m.metric_name === "llm_cost_usd")?.value || 0;
    const avgLag =
      systemMetrics
      ...
      Math.max(
        1,
        systemMetrics.filter((m) => m.metric_name?.startsWith("stream_lag:"))
          .length,
      );

    const getSignalStrength = (lag: number) => {
      if (lag < 100)
        return { icon: SignalHigh, color: "text-success", status: "Excellent" };
      if (lag < 1000)
        return { icon: SignalMedium, color: "text-warning", status: "Good" };
          ...
          <div className="flex flex-col justify-between">
            ...
            <div className="flex items-baseline gap-2">
              <span className="data-value text-slate-200">
                {Math.round(avgLag)}ms
              </span>
              <StatusChip
                status={
                  avgLag < 100 ? "success" : avgLag < 1000 ? "warning" : "error"
                }
...
const LogTerminal = memo(
  ({
    ...
    const logs = agentLogs.slice(0, 10).map((log, index) => {
      ...
      const primaryEdge =
        typeof log.primary_edge === "string"
          ? log.primary_edge
          : "No edge description";
      const confidence =
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
              ...
              <span className="bg-gray-900 dark:bg-white text-white dark:text-gray-900 px-3 py-1 rounded-md font-bold">
                Overview
              </span>
            </div>
