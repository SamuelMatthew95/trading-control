...
export function SignalsSidebar() {
  ...
  const load = async () => {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000/api"}/signals`,
    );
  ...
  const dismiss = async (id: string) => {
    ...
    try {
      await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000/api"}/signals/${id}/dismiss`,
      );
    } catch {
      setItems(previous);
    }
  ...
  return (
    <aside className="fixed right-0 top-0 z-20 h-full w-72 overflow-y-auto border-l border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-sans font-bold uppercase tracking-widest text-slate-900 dark:text-slate-100">
          Signals
        </h3>
        <button
          className="min-h-11 min-w-11 rounded-lg text-slate-500 transition-colors hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800"
      ...
      {(["urgent", "review", "info"] as const).map((priority) => (
        <section key={priority} className="mb-4">
          <h4 className="mb-2 text-xs font-sans font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
            {priority}
          </h4>
          {items
            ...
            .map((signal) => (
              <div
                key={signal.id}
                className="mb-2 rounded-xl border border-slate-200 p-3 dark:border-slate-800"
              >
