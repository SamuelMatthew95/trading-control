    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card className="bg-card border-border hover:border-muted-foreground/20 transition-colors">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
          <CardTitle className="text-xs font-sans font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
            System Status
          </CardTitle>
        ...
        <CardContent>
          <div className="flex items-center gap-2">
            <Badge
              variant={
                currentData?.data?.status === "healthy"
                  ? "default"
                  : "secondary"
              }
          ...
          <p className="text-xs font-sans text-slate-500 dark:text-slate-400 mt-2 leading-relaxed">
            {currentData?.data?.database_connected
              ? "Database Connected"
        ...
        <CardContent>
          ...
          <div className="mt-3">
            <Progress
              value={
                telemetry
                  ? Math.min(((telemetry.avg_latency_ms || 0) / 100) * 100, 100)
                  : 0
              }
              className="h-2"
            />
            <p className="text-xs font-sans text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
              Target: &lt;50ms
            </p>
          </div>
        </CardContent>
      ...
      <Card className="bg-card border-border hover:border-muted-foreground/20 transition-colors">
        ...
        <CardContent>
          ...
          <div className="flex items-center gap-2 mt-3">
            <Badge
              variant={
                telemetry?.error_rate === 0 ? "secondary" : "destructive"
              }
              className="text-xs px-2 py-1 bg-slate-100 text-slate-500"
            >
              Error Rate:{" "}
              {telemetry ? ((telemetry.error_rate || 0) * 100).toFixed(2) : "—"}
              %
            </Badge>
          </div>
        </CardContent>
