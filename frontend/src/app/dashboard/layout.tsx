...
export default function DashboardLayout({
  ...
  const { killSwitchActive, orders, positions, wsConnected, setKillSwitch } =
    useCodexStore();

  const dailyPnl = useMemo(() => {
    const realized = orders.reduce(
      (sum, order) => sum + (Number(order?.pnl) || 0),
      0,
    );
    ...
    <div className="flex min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      ...
      >
        <div className="flex h-12 items-center gap-2 border-b border-slate-200 px-4 dark:border-slate-800">
          ...
          <p className="text-sm font-bold uppercase tracking-widest font-sans text-slate-900 dark:text-slate-100">
            Trading Console
          </p>
        </div>
        ...
        <header className="h-12 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 sticky top-0 z-50">
          <div className="flex h-full items-center px-4">
            <div className="flex flex-1 items-center gap-2">
              ...
              <span className="text-sm font-bold uppercase tracking-widest font-sans text-slate-900 dark:text-white">
                {(() => {
                  if (pathname === "/dashboard") return "Overview";
                  if (pathname === "/dashboard/trading")
                    return "Trading Console";
                  if (pathname === "/dashboard/agents") return "Agent Monitor";
                  if (pathname === "/dashboard/learning") return "Learning";
                  if (pathname === "/dashboard/system") return "System";
                  return "Trading Console";
                })()}
              </span>
            </div>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div className="flex items-center gap-1.5">
                        ...
                        <span
                          className={cn(
                            "text-xs font-semibold uppercase tracking-wider font-sans hidden sm:inline",
