  ...
  const load = async () => {
    try {
      ...
      const [a, d] = await Promise.all([
        axios.get(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000/api"}/dashboard/pnl`,
        ),
        axios.get(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000/api"}/dashboard/run-summary`,
        ),
      ]);
      setPnl(a.data);
      setSummary(d.data);
  ...
  return (
    <div className="flex h-screen bg-background">
      ...
      <div className="flex-1 flex flex-col">
        <Header
          title="Performance"
          subtitle="Trading metrics and task performance"
        />
        <main className="flex-1 overflow-y-auto p-6">
          {/* Performance Metrics */}
          <section className="mb-8">
            <h2 className="text-[0.7rem] uppercase tracking-widest text-slate-500 mb-3">
              Performance Metrics
            </h2>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
              {[
                ...
                <div
                  key={String(k)}
                  className={`bg-card border border-border rounded-lg p-5 hover:border-muted-foreground/20 transition-colors ${
                    color === "blue"
                ...
                >
                  <div className="text-[0.7rem] uppercase tracking-widest text-slate-500 mb-2">
                    {k}
                  </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left pb-3 font-medium text-foreground">
                      Task
                    </th>
                    <th className="text-left pb-3 font-medium text-foreground">
                      Runs
                  ...
                  {(summary.items || []).length > 0 ? (
                    ...
                    (summary.items || []).map((r: any) => (
                      ...
                      >
                        ...
                        <td className="py-3 text-foreground">
                          {r.baseline_avg_steps}
                        </td>
                        <td className="py-3 text-foreground">{r.avg_pnl}</td>
                        <td className="py-3">
                          <a
                            href={`/film-room?task_type=${encodeURIComponent(r.task_slug)}`}
                      ...
                      <td colSpan={7} className="py-12">
                        <div className="flex flex-col items-center justify-center text-center">
                          ...
                          <p className="text-sm text-muted-foreground">
                            Task performance will appear here once the bot runs
                          </p>
                        </div>
                      </td>
                    </tr>
