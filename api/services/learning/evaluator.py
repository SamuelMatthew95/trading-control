...
class TradeEvaluator(BaseStreamConsumer):
    ...
    async def process(self, data: dict[str, Any]) -> None:
        ...
        async with AsyncSessionFactory() as session:
            ...
            await session.execute(
                text(
                    "INSERT INTO trade_performance (order_id, symbol, pnl, holding_secs, entry_price, exit_price, market_context, factor_attribution) VALUES (:order_id, :symbol, :pnl, :holding_secs, :entry_price, :exit_price, CAST(:market_context AS JSONB), CAST(:factor_attribution AS JSONB))"
                ),
                {
                    "order_id": order_id,
                    "symbol": symbol,
    ...
    async def _fetch_prior_trade(
        ...
        result = await session.execute(
            text(
                "SELECT o.side, o.qty, o.price, o.filled_at, tp.exit_price, tp.created_at FROM orders o LEFT JOIN trade_performance tp ON tp.order_id = o.id WHERE o.strategy_id = :strategy_id AND o.symbol = :symbol AND o.id != :order_id AND o.status = 'filled' ORDER BY COALESCE(o.filled_at, o.created_at) DESC LIMIT 1"
            ),
    ...
    async def _update_strategy_metrics(self, session, strategy_id: str) -> None:
        result = await session.execute(
            text(
                "SELECT tp.pnl FROM trade_performance tp JOIN orders o ON o.id = tp.order_id WHERE o.strategy_id = :strategy_id ORDER BY tp.created_at ASC"
            ),
            {"strategy_id": strategy_id},
        )
        ...
        if row is None:
            await session.execute(
                text(
                    "INSERT INTO strategy_metrics (strategy_id, win_rate, avg_pnl, sharpe, max_drawdown, updated_at) VALUES (:strategy_id, :win_rate, :avg_pnl, :sharpe, :max_drawdown, NOW())"
                ),
                params,
            )
        else:
            await session.execute(
                text(
                    "UPDATE strategy_metrics SET win_rate = :win_rate, avg_pnl = :avg_pnl, sharpe = :sharpe, max_drawdown = :max_drawdown, updated_at = NOW() WHERE strategy_id = :strategy_id"
                ),
                params,
            )
