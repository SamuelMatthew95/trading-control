...
class ExecutionEngine(BaseStreamConsumer):
    ...
    async def process(self, data: dict[str, Any]) -> None:
        ...
        idempotency_key = self._build_idempotency_key(
            strategy_id, symbol, side, order_timestamp
        )
        ...
        async with AsyncSessionFactory() as session:
            existing = await session.execute(
                text(
                    "SELECT id, status, broker_order_id, idempotency_key "
                    "FROM orders WHERE idempotency_key = :idempotency_key"
                ),
            ...
            try:
                inserted = await session.execute(
                    text(
                        "INSERT INTO orders (strategy_id, symbol, side, qty, price, status, idempotency_key, broker_order_id) "
                        "VALUES (:strategy_id, :symbol, :side, :qty, :price, 'pending', :idempotency_key, NULL) "
                        "RETURNING id"
                    ),
                    {
                ...
                await session.execute(
                    text(
                        "UPDATE orders "
                        "SET status = :status, broker_order_id = :broker_order_id, price = :fill_price, filled_at = :filled_at "
                        "WHERE id = :order_id"
                    ),
                    {
    ...
    def _build_idempotency_key(
        self, strategy_id: str, symbol: str, side: str, timestamp: datetime
    ) -> str:
    ...
    async def _upsert_position(
        ...
        if row is None:
            await session.execute(
                text(
                    "INSERT INTO positions (symbol, side, qty, entry_price, current_price, unrealised_pnl, strategy_id) "
                    "VALUES (:symbol, :side, :qty, :entry_price, :current_price, :unrealised_pnl, :strategy_id)"
                ),
                {
                    "symbol": symbol,
