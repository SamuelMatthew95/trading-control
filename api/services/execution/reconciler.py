        ...
        async with AsyncSessionFactory() as session:
            result = await session.execute(
                text(
                    ...
                    "WHERE status IN ('pending', 'partial') AND created_at < :cutoff"
                ),
                {"cutoff": cutoff},
            )
            ...
            for row in rows:
                ...
                await session.execute(
                    text(
                        "INSERT INTO order_reconciliation (order_id, discrepancy, resolved) "
                        "VALUES (:order_id, CAST(:discrepancy AS JSONB), true)"
                    ),
                    {
                        "order_id": row["id"],
                ...
                await session.execute(
                    text(
                        ...
                        "VALUES ('order_reconciled', CAST(:payload AS JSONB))"
                    ),
