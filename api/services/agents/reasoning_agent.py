...
class ReasoningAgent(BaseStreamConsumer):
    ...
    async def _search_vector_memory(
        ...
        query = text(
            "SELECT id, content, metadata_, outcome, 1 - (embedding <=> CAST(:embedding AS vector)) AS sim "
            "FROM vector_memory ORDER BY embedding <=> CAST(:embedding AS vector) LIMIT 5"
        )
        try:
    ...
    async def _call_reasoning_model(
        ...
        system_prompt = (
            ...
            "risk_factors, size_pct, stop_atr_x, rr_ratio, latency_ms, cost_usd, trace_id, fallback."
        )
        user_prompt = {
            "signal": data,
    ...
    async def _apply_fallback(
        ...
        if settings.LLM_FALLBACK_MODE == "reject_signal":
            ...
            action = reflection.get("sizing_recommendation", base_action)
        else:
        ...
        try:
            async with AsyncSessionFactory() as session:
                result = await session.execute(
                    text(
                        "SELECT payload FROM agent_logs WHERE log_type = 'reflection' "
        ...
        try:
            async with AsyncSessionFactory() as session:
                await session.execute(
                    text(
                        "INSERT INTO agent_runs (strategy_id, symbol, signal_data, action, confidence, primary_edge, "
                        "risk_factors, size_pct, stop_atr_x, rr_ratio, latency_ms, cost_usd, trace_id, fallback) "
                        "VALUES (:strategy_id, :symbol, CAST(:signal_data AS JSONB), :action, :confidence, :primary_edge, "
                        "CAST(:risk_factors AS JSONB), :size_pct, :stop_atr_x, :rr_ratio, :latency_ms, :cost_usd, :trace_id, :fallback)"
                    ),
                    {
                        "strategy_id": data.get("strategy_id"),
                        "symbol": data.get("symbol"),
            ...
            async with AsyncSessionFactory() as session:
                await session.execute(
                    text(
                        ...
                        "VALUES (:content, CAST(:embedding AS vector), CAST(:metadata AS JSONB), CAST(:outcome AS JSONB))"
                    ),
                    {
                        "content": content,
    ...
    async def _store_agent_log(
        ...
        payload = {**summary, "fallback_reason": fallback_reason}
        try:
            async with AsyncSessionFactory() as session:
                await session.execute(
                    text(
            ...
            async with AsyncSessionFactory() as session:
                await session.execute(
                    text(
                        ...
                        "VALUES (:date, :tokens_used, :cost_usd)"
                    ),
