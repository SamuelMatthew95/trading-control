    ...
    try:
        ...
        async with get_async_session() as session:
            row = (
                await session.execute(
                    select(
                        ...
                        func.coalesce(
                            func.sum(
                                case((Run.created_at >= today, Run.pnl), else_=0.0)
                            ),
                            0.0,
                        ).label("pnl_today"),
                        func.coalesce(
                        ...
                        func.coalesce(
                            func.avg(
                                case(
                                    (
                                        and_(
                                            Run.created_at >= thirty_days,
                                            TaskTypeBaseline.baseline_slippage.is_not(
                                                None
                                            ),
                                            Run.actual_slippage.is_not(None),
                                        ),
                                        TaskTypeBaseline.baseline_slippage
                                        - Run.actual_slippage,
                                    ),
                                    else_=None,
                                )
            ...
            execution_cost = float(
                (
                    await session.execute(
                        select(func.coalesce(func.sum(TraceStep.token_cost_usd), 0.0))
                    )
                ).scalar()
                or 0.0
            )
        ...
        pct = (
            0.0
            if pnl_yesterday == 0
            else ((pnl_today - pnl_yesterday) / abs(pnl_yesterday)) * 100
        )
...
async def dashboard_learning_velocity(
    ...
    try:
        ...
        async with get_async_session() as session:
            pass_rows = (
                await session.execute(
                    select(
                        ...
                        func.count(Run.id),
                        func.sum(case((Run.status == "won", 1), else_=0)),
                    )
            ...
            coh_rows = (
                await session.execute(
                    select(
                        func.date(Run.created_at),
                        func.avg(Run.reasoning_coherence_score),
                    )
                    .where(Run.created_at >= since)
                    .group_by(func.date(Run.created_at))
                )
            ...
            coh_map = {
                str(d): (float(c) * 10 if c is not None else None) for d, c in coh_rows
            }
            ...
            annotations_this_week = int(
                (
                    await session.execute(
                        select(func.count(VectorMemoryRecord.id)).where(
                            VectorMemoryRecord.created_at >= now - timedelta(days=7)
                        )
                    )
                ).scalar()
                or 0
            ...
            good, total = (
                await session.execute(
                    select(
                        func.sum(case((Run.pnl > 0, 1), else_=0)),
                        func.count(TraceStep.id),
                    )
                    .join(Run, Run.id == TraceStep.run_id)
                    .where(
                        TraceStep.step_type == "skipped_by_memory_guard",
                        TraceStep.created_at >= now - timedelta(days=7),
                    )
                )
            ).one()
            verified = (
                await session.execute(
                    select(
                        func.avg(
                            func.julianday(VectorMemoryRecord.correction_verified_at)
                            - func.julianday(VectorMemoryRecord.created_at)
                        )
                    ).where(VectorMemoryRecord.correction_verified_at.is_not(None))
                )
            ).scalar()
                ...
                (
                    await session.execute(
                        select(func.count(Run.id)).where(
                            ...
                            Run.scoring_status.in_(["pending", "failed"]),
                        )
                    )
                ).scalar()
                or 0
        ...
        learning_data = LearningVelocityResponse(
            ...
            avg_sessions_to_correction=(
                None if verified is None else round(float(verified), 2)
            ),
            memory_guard_effectiveness_pct=round(guard_pct, 2),
            scoring_lag_warning=(
                total_recent >= 10
                and pending_or_failed > 5
                and pending_or_failed / total_recent > 0.2
            ),
        )

        return StandardResponse(
            success=True, data=learning_data.model_dump()
        ).model_dump()
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get learning velocity data: {str(e)}"
        )


@router.get("/dashboard/health-signals")
async def dashboard_health_signals(
    response: Response, reference_dt: Optional[datetime] = None
):
    try:
        now = reference_dt or datetime.utcnow()
        async with get_async_session() as session:
                ...
                (
                    await session.execute(
                        select(
                            func.coalesce(
                                func.avg(
                                    (TraceStep.tokens_used * 1.0)
                                    / func.nullif(TraceStep.context_limit, 0)
                                ),
                                0.0,
                            )
                        ).where(
                            TraceStep.context_limit.is_not(None),
                            TraceStep.tokens_used.is_not(None),
                        )
                    )
                ).scalar()
                or 0.0
            ...
            recent_runs = (
                (
                    await session.execute(
                        select(Run.id).where(Run.created_at >= now - timedelta(days=7))
                    )
                )
                .scalars()
                .all()
            ...
            for rid in recent_runs:
                tools = (
                    (
                        await session.execute(
                            select(TraceStep.tool_name)
                            .where(TraceStep.run_id == rid)
                            .order_by(TraceStep.id.asc())
                        )
                    )
                    .scalars()
                    .all()
                ...
                for i in range(1, len(tools)):
                    if tools[i] and tools[i] == tools[i - 1]:
                        ...
                        streak = 1
            thrash_rate = (
                0.0 if not recent_runs else thrash_runs / len(recent_runs) * 100
            )
            errors = int(
                (
                    await session.execute(
                        select(func.count(func.distinct(TraceStep.run_id))).where(
                            TraceStep.step_type == "error"
                        )
                    )
                ).scalar()
                or 0
            ...
            constraint = int(
                (
                    await session.execute(
                        select(func.count(TraceStep.id)).where(
                            TraceStep.step_type == "constraint_violation",
                            TraceStep.created_at >= now - timedelta(days=7),
                        )
                    )
                ).scalar()
            ...
            ghost_delta = float(
                (
                    await session.execute(
                        select(
                            func.coalesce(
                                func.avg(Run.actual_slippage - Run.ghost_slippage), 0.0
                            )
                        ).where(Run.ghost_run_id.is_not(None))
                    )
                ).scalar()
                or 0.0
            )
        ...
        health_signals = {
            "items": [
                HealthSignalView(
                    key="context_saturation",
                    label="Context saturation",
                    value=f"{context_ratio*100:.1f}%",
                    status="red" if context_ratio > 0.75 else "green",
                    interpretation="High means prompts are near token ceiling.",
                ).model_dump(),
                HealthSignalView(
                    key="tool_thrashing_rate",
                    label="Tool thrashing rate",
                    value=f"{thrash_rate:.1f}%",
                    status="red" if thrash_rate > 20 else "amber",
                    interpretation="Repeated tool loops indicate unstable planning.",
                ).model_dump(),
                HealthSignalView(
                    key="recovery_rate",
                    label="Recovery rate",
                    value=f"{recovery_rate:.1f}%",
                    status="green" if recovery_rate > 65 else "amber",
                    interpretation="How often failures self-correct before final decision.",
                ).model_dump(),
                HealthSignalView(
                    key="constraint_violations",
                    label="Constraint violations",
                    value=str(constraint),
                    status="red" if constraint > 0 else "green",
                    interpretation="Any violation should be reviewed immediately.",
                ).model_dump(),
                HealthSignalView(
                    key="ghost_path_delta",
                    label="Ghost path delta",
                    value=f"{ghost_delta:.3f}",
                    status="green" if ghost_delta > 0 else "amber",
                    interpretation="Positive means current path beats ghost baseline.",
                ).model_dump(),
                HealthSignalView(
                    key="memory_guard_hits",
                    label="Memory guard hits",
                    value=str(guard_hits),
                    status="blue",
                    interpretation="Informational count of blocked risky tool calls.",
                ).model_dump(),
            ]
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get health signals: {str(e)}"
        )


@router.get("/dashboard/run-summary")
async def dashboard_run_summary(
    response: Response, reference_dt: Optional[datetime] = None
):
    try:
        now = reference_dt or datetime.utcnow()
        ...
        async with get_async_session() as session:
                        ...
                        (
                            ...
                            * 100.0
                            / func.nullif(
                                func.sum(
                                    case(
                                        (Run.status.in_(["won", "failed"]), 1), else_=0
                                    )
                                ),
                                0,
                            )
            ...
            for task_type, runs_7d, win_rate, avg_steps, avg_pnl in groups:
                baseline = float(
                    (
                        await session.execute(
                            select(func.coalesce(func.avg(Run.step_count), 0.0)).where(
                ...
                daily = (
                    await session.execute(
                        select(
                            func.date(Run.created_at),
                            func.coalesce(func.avg(Run.pnl), 0.0),
                        )
                        .where(Run.task_type == task_type, Run.created_at >= since)
                        .group_by(func.date(Run.created_at))
                    )
                ).all()
                ...
                sparkline = [
                    round(
                        dmap.get((now - timedelta(days=6 - i)).date().isoformat(), 0.0),
                        2,
                    )
                    for i in range(7)
                ]
                items.append(
                    RunSummaryRowView(
                        ...
                        avg_steps=round(float(avg_steps or 0.0), 2),
                        baseline_avg_steps=round(baseline, 2),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get run summary: {str(e)}"
        )
...
async def _upsert_signal(
    ...
    *,
    ...
    sid = hashlib.sha1(
        f"{priority}|{signal_type}|{source_entity_id}".encode()
    ).hexdigest()[:24]
    if (
        await session.execute(select(Signal.id).where(Signal.id == sid))
    ...
    async with get_async_session() as session:
        bad_runs = (
            (
                await session.execute(
                    select(Run).where(
                        Run.created_at >= now - timedelta(hours=1), Run.pnl < -500
                    )
                )
            )
            .scalars()
        ...
        for run in bad_runs:
            await _upsert_signal(
                session,
                priority="urgent",
                signal_type="large_loss",
                source_entity_id=str(run.id),
                message=f"Run {run.id} posted a large loss ({run.pnl:.2f}).",
        ...
        constraints = int(
            (
                await session.execute(
                    select(func.count(TraceStep.id)).where(
                        TraceStep.step_type == "constraint_violation",
                        TraceStep.created_at >= _utc_midnight(now, 0),
                    )
                )
            ).scalar()
            or 0
        ...
        if constraints > 0:
            await _upsert_signal(
                ...
                source_entity_id="today",
                message=f"{constraints} constraint violations detected today.",
                action_label="Flag",
                action_type="flag",
            )
        ...
        low_conf = (
            (
                await session.execute(
                    select(Insight)
                    .where(Insight.confidence < 0.6, Insight.dismissed.is_(False))
                    .limit(50)
                )
            )
            .scalars()
        ...
        for ins in low_conf:
            await _upsert_signal(
                session,
                priority="review",
                signal_type="low_confidence_insight",
                source_entity_id=str(ins.id),
                message=f"Insight {ins.id} needs more data (conf {ins.confidence:.2f}).",
                action_label="Reinforce",
                action_type="reinforce",
                run_id=str(ins.run_id),
            )
            ...
            (
                await session.execute(
                    select(StrategyDNA).where(
                        StrategyDNA.updated_at >= now - timedelta(hours=24)
                    )
                )
            )
            .scalars()
        ...
        for row in dna:
            await _upsert_signal(
                session,
                priority="info",
                signal_type="dna_state_change",
                source_entity_id=row.rule_key,
                message=f"DNA segment {row.rule_key} changed state to {row.state}.",
        ...
        async with get_async_session() as session:
            rows = (
                (
                    await session.execute(
                        select(Signal)
                        .where(Signal.dismissed.is_(False))
...
async def dismiss_signal(signal_id: str):
    try:
        async with get_async_session() as session:
            ...
            await session.flush()
            return StandardResponse(
                success=True, data={"status": "dismissed", "id": signal_id}
            ).model_dump()
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to dismiss signal: {str(e)}"
        )
    ...
    try:
        async with get_async_session() as session:
            ...
            scoring_failed = int(
                (
                    await session.execute(
                        select(func.count(Run.id)).where(Run.scoring_status == "failed")
                    )
                ).scalar()
                or 0
            )
            ...
            scoring_abandoned_count = int(
                (
                    await session.execute(
                        select(func.count(Run.id)).where(
                            Run.scoring_abandoned_at.is_not(None)
                        )
                    )
                ).scalar()
                or 0
            ...
            last_prompt_rebuild = (
                await session.execute(
                    select(func.max(VectorMemoryRecord.created_at)).where(
                        VectorMemoryRecord.store_type == "prompt-cache"
                    )
                )
            ).scalar()
            last_successful_score_at = (
                await session.execute(
                    select(func.max(Run.updated_at)).where(
                        Run.scoring_status == "scored"
                    )
                )
            ).scalar()
        ...
        return StandardResponse(
            success=True, data=health_data.model_dump()
        ).model_dump()
    except Exception as e:
        raise HTTPException(
...
async def dashboard_options():
    return StandardResponse(
        success=True,
        data={"message": "Dashboard endpoints support GET, POST, and OPTIONS"},
    ).model_dump()
...
async def _upsert_signal(
    ...
    existing = (
        await session.execute(
            select(Signal).where(
                Signal.source_entity_id == source_entity_id,
