...
async def get_signals(response: Response):
    async with get_async_session() as session:
        ...
        response.headers["Cache-Control"] = "no-store"
        return {"items": [{"id": r.id, "priority": r.priority, "message": r.message, "action_label": r.action_label, "action_type": r.action_type, "run_id": r.run_id, "created_at": r.created_at, "dismissed": r.dismissed} for r in rows]}
