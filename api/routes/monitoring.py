...
async def monitoring_logs(limit: int = 50) -> Dict[str, Any]:
    ...
    try:
        ...
        return StandardResponse(
            success=True, data={"logs": [], "limit": limit}
        ).model_dump()
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get monitoring logs: {str(e)}"
        )
...
async def monitoring_options() -> Dict[str, Any]:
    ...
    return StandardResponse(
        success=True, data={"message": "Monitoring endpoints support GET and OPTIONS"}
    ).model_dump()
