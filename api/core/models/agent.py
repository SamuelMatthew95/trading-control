...
from sqlalchemy import Column, String, DateTime, Integer, Text, Float, Index, Numeric, ForeignKey, Enum, CheckConstraint
...
class AgentPool(Base):
    ...
    agent_type = Column(Enum('analysis', 'execution', 'learning', 'monitoring', name='agent_type'), nullable=False, index=True)
    description = Column(Text, nullable=True)
    config = Column(MutableDict.as_mutable(JSONB), default=dict, server_default=text("'{}'::jsonb"))
    capabilities = Column(MutableDict.as_mutable(JSONB), default=dict, server_default=text("'[]'::jsonb"))
    status = Column(Enum('active', 'inactive', 'archived', name='pool_status'), nullable=False, default='active', index=True)
    version = Column(String, nullable=False, default='1.0.0')
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
    
    __table_args__ = (
        Index('idx_agent_pool_type_status', 'agent_type', 'status'),
    )
...
class AgentRun(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, server_default=text("gen_random_uuid()"))
    agent_id = Column(UUID(as_uuid=True), ForeignKey('agent_pool.id', ondelete='CASCADE'), nullable=False, index=True)
    trace_id = Column(String, nullable=False, index=True)
    run_type = Column(Enum('analysis', 'execution', 'learning', name='run_type'), nullable=False, index=True)
    trigger_event = Column(String, nullable=True)
    input_data = Column(MutableDict.as_mutable(JSONB), nullable=False, server_default=text("'{}'::jsonb"))
    ...
    status = Column(Enum('running', 'completed', 'failed', 'cancelled', name='run_status'), nullable=False, default='running', index=True)
...
class AgentLog(Base):
    ...
    log_level = Column(Enum('debug', 'info', 'warning', 'error', name='log_level'), nullable=False, default='info', index=True)
    message = Column(Text, nullable=False)
    step_name = Column(String, nullable=True, index=True)
    step_data = Column(MutableDict.as_mutable(JSONB), default=dict, server_default=text("'{}'::jsonb"))
...
class AgentGrades(Base):
    ...
    agent_id = Column(UUID(as_uuid=True), ForeignKey('agent_pool.id', ondelete='CASCADE'), nullable=False, index=True)
    agent_run_id = Column(UUID(as_uuid=True), ForeignKey('agent_runs.id', ondelete='CASCADE'), nullable=False, index=True)
    grade_type = Column(Enum('accuracy', 'efficiency', 'safety', 'overall', name='grade_type'), nullable=False, index=True)
    score = Column(Numeric(3, 2), nullable=False)
    metrics = Column(MutableDict.as_mutable(JSONB), default=dict, server_default=text("'{}'::jsonb"))
