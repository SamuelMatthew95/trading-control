...
class Order(Base):
    ...
    order_type = Column(Enum('market', 'limit', 'stop', 'stop_limit', name='order_type'), nullable=False)
    ...
    status = Column(Enum('pending', 'filled', 'cancelled', 'rejected', name='order_status'), nullable=False, default='pending', index=True)
    exchange = Column(String, nullable=True, index=True)
    commission = Column(Numeric(18, 8), server_default="0", nullable=False)
    order_metadata = Column(MutableDict.as_mutable(JSONB), nullable=False, server_default=text("'{}'::jsonb"))
