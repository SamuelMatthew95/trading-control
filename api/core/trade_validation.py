...
class TradeRelationshipValidator(BaseModel):
    ...
    db_trade_id: str = Field(..., description="Database trade identifier")
    parent_trade_id: str | None = Field(None, description="Parent trade identifier")
    child_trade_id: str | None = Field(None, description="Child trade identifier")
    symbol: str = Field(..., description="Trading symbol")
    relationship_type: str = Field(
        ..., regex="^(BUY_SELL|OPEN_POSITION)$", description="Relationship type"
    )

    @validator("relationship_type")
    def validate_relationship(self, v):
...
class StrictTradeValidator:
    ...
    def validate_trade_fields(self, trade_data: dict[str, Any]) -> RequiredTradeFields:
        ...
        try:
            ...
            raise TradeValidationError(error_msg) from e

    def validate_trade_relationships(
        self, trade_data: dict[str, Any]
    ) -> TradeRelationshipValidator:
