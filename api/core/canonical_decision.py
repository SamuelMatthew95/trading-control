...
class RiskPhase(BaseModel):
    ...
    max_position_size: Optional[Decimal] = Field(None, gt=0, description="Max position size")
...
class ExecutionPhase(BaseModel):
    ...
    position_id: Optional[str] = Field(None, description="Target position ID for closing")
...
class TradeDecision(BaseModel):
    ...
    processing_duration_ms: Optional[int] = Field(None, ge=0, description="Processing duration")
    ...
    def validate_agent_chain(cls, v):
        if len(v) < 3:
...
class CanonicalDecisionStore:
    ...
    def get_decision(self, signal_id: str) -> Optional[TradeDecision]:
